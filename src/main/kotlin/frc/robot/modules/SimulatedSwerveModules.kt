...
public class SimulatedSwerveModule : ISwerveModule {
    ...
    constructor(fakeDrivePortA: Int, fakeDrivePortB: Int, fakeTurnPortA: Int, fakeTurnPortB: Int) {
        this.driveMotor = FlywheelSim(
            LinearSystemId.identifyVelocitySystem(DriveConstants.SwerveRamsete.kv, DriveConstants.SwerveRamsete.ka),
  ...
  public override fun setDesiredState(desiredState: SwerveModuleState, preventTurn: Boolean, slow: Boolean, pid: Boolean) {
    ...
    var newTurnOutput: Double = /*if ((Math.abs(desiredState.speedMetersPerSecond)) <= (DriveConstants.SwerveRamsete.maxVelocity * 0.01)) this.turnOutput 
  ...
  public override fun simulationPeriodic(dt: Double) {
    ...
    driveMotor.setInputVoltage(driveOutput / DriveConstants.SwerveRamsete.maxVelocity * RobotController.getBatteryVoltage());
