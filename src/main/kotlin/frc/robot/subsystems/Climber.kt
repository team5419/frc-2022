...
class Climber(tab: ShuffleboardTab) : SubsystemBase() {
    ...
    init {
        for(i in 0..motors.size - 1) {
                motors[i].motor.apply {
            ...
            motors[i].motor.setInverted(motors[i].inverted);
        }
    ...
    fun left(pair: Int): Double {
        return 0.0;
        //return motors[pair].left.sensor.getValue().toDouble()
    }

    fun right(pair: Int): Double {
        return 0.0;
        //return motors[pair].right.sensor.getValue().toDouble()
    }
    ...
    fun getAllVelocities() : List<Double> {
        ...
        for(pair in motors) {
            returning.add(pair.motor.getSelectedSensorVelocity());
        }
    ...
    public fun stop() {
        for(pair in motors) {
            pair.motor.set(ControlMode.PercentOutput, 0.0)
        }
    }
    ...
    public fun setPairVelocity(pair: Int, throttle: Double) {
        if(motors[pair].previousThrottle == throttle) {
            return;
        }
        motors[pair].previousThrottle = throttle;
        val f: Double = 21000.0;
        motors[pair].motor.set(ControlMode.Velocity, (withDeadband(-throttle, 0.1)) * f);
        lastLeftSetpoint = (withDeadband(-throttle, 0.1)) * f;
    ...
    //     motors[pair].left.motor.set(ControlMode.Velocity, (withDeadband(-throttle - leftturn, 0.1)) * f);
    //     //m_controller.setGoal(left(pair));
    //     //motors[pair].right.motor.set(ControlMode.Velocity, m_controller.calculate(right(pair)));
    //     // note from theo:
    ...
    //     if(motors[pair].previousThrottle == throttle && motors[pair].previousTurn == turn) {
    //         return;
    //     }
    //     motors[pair].previousThrottle = throttle;
    //     motors[pair].previousTurn = turn;
    //     val f: Double = 15000.0;
    ...
    //     if((_left < 0 && left(pair) < motors[pair].left.max) || (_left > 0 && left(pair) > motors[pair].left.min)) {
    //         motors[pair].left.motor.set(ControlMode.Velocity, _left);
    //     }
    //     if((_right < 0 && right(pair) < motors[pair].right.max) || (_right > 0 && right(pair) > motors[pair].right.min)) {
    //         motors[pair].right.motor.set(ControlMode.Velocity, _right);
    //     }
    //     // note from theo:
    ...
    // public fun setmotorslightAdjust(pair: Int, throttle: Double, turn: Double = 0.0) {
    //     val f: Double = 15000.0;
    ...
    //     motors[pair].left.motor.set(ControlMode.Velocity, velocity  - leftturn);
    //     motors[pair].right.motor.set(ControlMode.Velocity, velocity + rightturn);
    //     // note from theo:
    ...
    public fun toMax(pair: Int) {
        motors[pair].motor.set(ControlMode.Position, motors[pair].max);
    }

    public fun toMin(pair: Int) {
        motors[pair].motor.set(ControlMode.Position, motors[pair].min);
    }

    public fun setPair(pair: Int, throttle: Double) {
        val f: Double = 0.5;
        motors[pair].motor.set(ControlMode.PercentOutput, withDeadband(-throttle, 0.001) * f);
    }
