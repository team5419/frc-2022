...
class Climber(tab: ShuffleboardTab) : SubsystemBase() {
    ...
    fun left(arm: Int): Double {
        return 0.0;
        //return pairs[pair].left.sensor.getValue().toDouble()
    }

    fun right(arm: Int): Double {
        return 0.0;
    ...
    fun getAllVelocities() : List<Double> {
        ...
        returning.add(leftArm.getSelectedSensorVelocity());
        returning.add(rightArm.getSelectedSensorVelocity());
    ...
    public fun setArmVelocity(motor: Int, throttle: Double, turn: Double = 0.0) {
        if(leftArm.previousThrottle == throttle && leftArm.previousTurn == turn
           rightArm.previousThrottle == throttle && rightArm.previousTurn == turn) {
            return;
        }
        leftArm.previousThrottle = throttle;
        leftArm.previousTurn = turn;
        rightArm.previousThrottle = throttle;
        rightArm.previousTurn = turn;
        ...
        leftArm.set(ControlMode.Velocity, (withDeadband(-throttle - leftturn, 0.1)) * f);
        rightArm.set(ControlMode.Velocity, (withDeadband(-throttle + rightturn, 0.1)) * f);
    ...
    public fun toMax(pair: Int) {
        leftArm.set(ControlMode.Position, pairs[pair].left.max);
        rightArm.set(ControlMode.Position, pairs[pair].right.max);
    }

    public fun toMin(pair: Int) {
        leftArm.set(ControlMode.Position, pairs[pair].left.min);
        rightArm.set(ControlMode.Position, pairs[pair].right.min);
    }

    public fun setPair(pair: Int, throttle: Double) {
        val f: Double = 0.5;
        leftArm.set(ControlMode.PercentOutput, withDeadband(-throttle, 0.001) * f);
        rightArm.set(ControlMode.PercentOutput, withDeadband(-throttle, 0.001) * f);
    }
